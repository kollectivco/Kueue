...
class EventMetaBoxes {
    ...
    public function render_events_meta_box( $post ) {
        ...
        // 2. Wrap loading in try-catch to reveal fatal errors if possible (though static calls won't catch easily if class missing)
        try {
            // Fetch All Data
            $data = [
            ...
            $organizers = class_exists('\KueueEvents\Core\Modules\Vendors\OrganizerRepository') 
                ? \KueueEvents\Core\Modules\Vendors\OrganizerRepository::get_all() : [];
            
            $sms_accounts = class_exists('\KueueEvents\Core\Modules\Gateways\GatewayManager') 
                ? \KueueEvents\Core\Modules\Gateways\GatewayManager::get_accounts( 'sms' ) : [];
            
            $whatsapp_accounts = class_exists('\KueueEvents\Core\Modules\Gateways\GatewayManager') 
                ? \KueueEvents\Core\Modules\Gateways\GatewayManager::get_accounts( 'whatsapp' ) : [];
            
            $seating_maps = class_exists('\KueueEvents\Core\Modules\Seating\SeatingRepository') 
                ? \KueueEvents\Core\Modules\Seating\SeatingRepository::get_all_maps() : [];
            
            $venues = get_posts([ 'post_type' => 'kq_venue', 'numberposts' => -1, 'post_status' => 'any' ]) ?: [];
            ...
            if ( file_exists( $view_path ) ) {
                ...
                echo '<div class="notice notice-error"><p>' . sprintf( __( 'Event meta box view file not found at %s', 'kueue-events-core' ), $view_path ) . '</p></div>';
            }
