...
use FooEventsPOS\Stripe\StripeObject;
...
abstract class Util
{
            ...
            if (isset($resp['object']) && \is_string($resp['object'])
                ...
                if ('v2' === $apiMode && ('v2.core.event' === $resp['object'])) {
                    ...
                    if (\array_key_exists('type', $resp) && \array_key_exists($resp['type'], $eventTypes)) {
                        ...
                        $class = \FooEventsPOS\Stripe\V2\Event::class;
                    }
                }
                ...
                $class = \FooEventsPOS\Stripe\V2\Collection::class;
            } else {
        ...
        foreach ($properties as $key => $property) {
            if (\array_key_exists($property->getName(), $json)) {
                if ('related_object' === $property->getName()) {
                    $related_object = new \FooEventsPOS\Stripe\RelatedObject();
                    ...
                    $reason = new \FooEventsPOS\Stripe\Reason();
    ...
    {
        if ($h instanceof \FooEventsPOS\Stripe\ApiResource) {
            return $h->id;
        }
        if (static::isList($h)) {
            $results = [];
