...
/*
 * Plugin Name: Additional SMS & WhatsApp Gateways
 * Description: Additional SMS & WhatsApp Gateways for Digits and WPNotif
 * Version: 10.8
 * Plugin URI: https://www.unitedover.com/
...
<message><sender>' . $sender . '</sender><text>' . $messagetemplate . '</text><abonent phone="' . $phone . '" /></message></request>';
        ...
        case 100:
            ...
            return \SMSGateway\Semaphore::sendSMS(
                $gateway_fields, $mobile, $messagetemplate, $testCall
            );
        case 101:
        ...
        case 201:
            $gateway_fields = get_option($option_slug . '_turbosmsua');
            require_once 'gateways/turbosmsua.php';
            return \SMSGateway\TurboSMSUA::sendSMS(
                $gateway_fields,
                $countrycode . $mobile, $messagetemplate,
                $testCall
            );
        case 202:
            $gateway_fields = get_option($option_slug . '_yourbulksms');
            require_once 'gateways/yourbulksms.php';
            return \SMSGateway\YourBulkSMS::sendSMS(
                $gateway_fields,
                $countrycode . $mobile, $messagetemplate,
                $testCall
            );
        case 203:
            $gateway_fields = get_option($option_slug . '_birdgateway');
            require_once 'gateways/bird.php';
            return \SMSGateway\Bird::sendSMS(
                $gateway_fields,
                $countrycode . $mobile, $messagetemplate,
                $testCall
            );
        case 204:
            $gateway_fields = get_option($option_slug . '_smstoolssk');
            require_once 'gateways/smstoolssk.php';
            return \SMSGateway\SMSToolsSK::sendSMS(
                $gateway_fields,
                $countrycode . $mobile, $messagetemplate,
                $testCall
            );
...
)
{
    ...
    switch ($gateway_id) {
        ...
        case 17:
            $gateway_fields = array('account_sid' => $options['twilio_new_account_sid'],
                'auth_token' => $options['twilio_new_auth_token'],
                'whatsappnumber' => $options['twilio_new_whatsappnumber'],
                'mssid' => $options['twilio_new_mssid']
            );
            unitedover_load_gateways_sdks();
            require_once 'gateways/wa/twilio.php';
            return \SMSGateway\wa\Whatsapp_Twilio::process_whatsapp($gateway_fields, $phone, $message, $testCall);
        case 18:
            $gateway_fields = array(
                'accesskey'   => $options['bird_accesskey'],
                'channel_id'  => $options['bird_channel_id'],
                'workspace_id' => $options['bird_workspace_id'],
                'language'    => isset($options['bird_template_language']) ? $options['bird_template_language'] : 'en',
                'project_id'  => isset($options['bird_project_id']) ? $options['bird_project_id'] : '',
            );
            require_once 'gateways/wa/bird.php';
            return \SMSGateway\wa\Whatsapp_Bird::process_whatsapp($gateway_fields, $phone, $message, $testCall);
        default:
            return false;
    }
...
)
{
    ...
    switch ($gateway_id) {
        case 2:
            $gateway_fields = get_option($option_slug . '_' . $prefix . 'twilio');
            unitedover_load_gateways_sdks();
            require_once 'gateways/wa/twilio.php';
            return \SMSGateway\wa\Whatsapp_Twilio::process_whatsapp($gateway_fields, $phone, $message, $testCall);
        case 3:
        ...
        case 17:
            ...
            return \SMSGateway\wa\Whatsapp_Twilio::process_whatsapp($gateway_fields, $phone, $message, $testCall);
        case 18:
            $gateway_fields = get_option($option_slug . '_' . $prefix . 'birdgateway');
            require_once 'gateways/wa/bird.php';
            return \SMSGateway\wa\Whatsapp_Bird::process_whatsapp($gateway_fields, $phone, $message, $testCall);
        default:
            return false;
...
function untdovr_error_use_template_messages($test_call)
{
    if (!$test_call) {
        return;
    }
