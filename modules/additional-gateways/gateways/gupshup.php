...
class GupShup
{
    ...
    public static function process_whatsapp($gateway_fields, $mobile, $message, $testCall)
    {
        ...
        $template_ids = array('template-id', 'template-type', 'media-link');
        $params_values = array();
        ...
        if (isset($gateway_fields['template_id'])) {
            ...
            if (empty($whatsapp) || !is_array($whatsapp)) {
                untdovr_error_use_template_messages($testCall);
                return false;
            }
        ...
        curl_setopt($ch, CURLOPT_URL, 'https://api.gupshup.io/wa/api/v1/template/msg');
