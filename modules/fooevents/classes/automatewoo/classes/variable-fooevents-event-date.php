...
/**
 * Attendee event date AutomateWoo variable.
 */
class Variable_FooEvents_Event_Date extends AutomateWoo\Variable_Abstract_Datetime {
	 ...
	 */
	protected $name = 'fooevents.event_date';
	...
	public function load_admin_details() {
		...
		$this->description = __( 'Displays the event date.', 'woocommerce-events' );
	...
	public function get_value( $ticket, $parameters, $workflow ) {
		...
		if ( 'select' === $event_type ) {
			...
			if ( ! empty( $woocommerce_events_select_date ) ) {
				...
				return $this->format_datetime( $event_date, $parameters );

			}
		} else {

			return $this->format_datetime( $ticket['WooCommerceEventsDate'], $parameters );
...
return 'Variable_FooEvents_Event_Date';
